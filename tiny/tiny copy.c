...
int parse_uri(char *uri, char *filename, char *cgiargs) 
{
  ...
  if (!strstr(uri, "cgi-bin")) {  /* Static content */ //line:netp:parseuri:isstatic
    ...
    // if (strstr(uri, "add"))  /* Static content */ //line:netp:parseuri:isstatic
    //   strcat(filename, "add.html");                             //line:netp:parseuri:endconvert1
    
    // if (uri == "add")                   //line:netp:parseuri:slashcheck
    //   strcat(filename, "home.html"); 
...
void serve_static(char* method, int fd, char *filename, int filesize)
{
    ...
    printf("hello~2222");
    if (!strcasecmp(method, "HEAD")) {       // 헤더 메소가 들어왔으면 그냥 나가기 
      return;
    }
...
void serve_dynamic(char* method, int fd, char *filename, char *cgiargs) 
{
    ...
    if (!strcasecmp(method, "HEAD")) {                     //line:netp:doit:beginrequesterr
      return;
    }
