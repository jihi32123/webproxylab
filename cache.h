...
/*
    ...
    1. 캐시 구조체
        -> 링크드 리스트 (엘리먼트는 노드) root, tail 포인터 필요 (LRU 구현을 위해), 지금 보유 데이터 크기 필요
        -> 노드 (key : GET http://local../home.html HTTP/1.1), value(응답 데이터), 링크드 리스트 포인터 (next, prev) 필요

    3. 데이터 찾기 (find_value) : 시간이 남으면 캐시로 구현하자
        1. key를 입력하면 데이터 찾기 
    ...
    2. 데이터 저장
        ...
        3. 가장 오래 된 것을 찾아서 삭제하자
    3. 데이터 삭제 (무조건 가장 오래된 것만 삭제한다고 가정)
        1. 값을 삭제하고, 데이터의 앞, 뒤를 연결해준다.
...
int init_cache(cache *target_cache);
